...
class nconv(nn.Module):
    def __init__(self,inputsize,num_nodes,input_len,pred_len, topk=5, mask_bias_w=0.002, tri_bias_w=0.001, spatial_mask_update=True):
        ...
        self.topk = topk
        self.mask_bias_w = mask_bias_w
        self.tri_bias_w = tri_bias_w
        self.spatial_mask_update = spatial_mask_update
    ...
    def forward(self,x, A,mask,k):
        ...
        # 将mask的时间维统一为构造时约定的inputsize，避免dilation导致的长度不一致
        if T != self.inputsize:
            # 仅在时间维做插值到self.inputsize，节点数N保持不变
            mask = F.interpolate(mask.float(), size=(N, self.inputsize), mode='nearest')
            T = self.inputsize

        mask_projection = self.mlp(mask.float())
        mask_weight = torch.matmul(mask_projection, mask_projection.transpose(-2, -1))
        mask_weight = mask_weight + self.tri_bias_w * self.triu_matrix * self.alpha1.triu() + self.tri_bias_w * self.tril_matrix * self.alpha2.tril()

        mask_weight = torch.sigmoid(mask_weight / torch.sqrt(torch.tensor(10.0)))
        A = A.unsqueeze(0).unsqueeze(0) + self.mask_bias_w * mask_weight

        if self.spatial_mask_update:
            _, topk_A = torch.topk(A, k=self.topk, dim=-1, largest=True)
...
class mixprop(nn.Module):
    def __init__(self,c_in,c_out,gdep,dropout,alpha,inputsize,num_nodes,input_len,pred_len, topk=5, mask_bias_w=0.002, tri_bias_w=0.001, spatial_mask_update=True):
        ##16 16 2 0.3 0.05
        super(mixprop, self).__init__()
        self.nconv = nconv(inputsize,num_nodes,input_len,pred_len, topk=topk, mask_bias_w=mask_bias_w, tri_bias_w=tri_bias_w, spatial_mask_update=spatial_mask_update)
...
class dilated_inception(nn.Module):
    def __init__(self, cin, cout,kernel_set, dilation_factor=2, temporal_partial_conv=True, temporal_mask_update=True):

        super(dilated_inception, self).__init__()
        self.tconv = nn.ModuleList()
        self.mconv=nn.ModuleList()
        self.kernel_set = kernel_set#[2,3,6,7]
        self.temporal_partial_conv = temporal_partial_conv
        self.temporal_mask_update = temporal_mask_update
    ...
    def forward(self,input,mask):
        x = []
        mask_list=[]
        # 仅取mask的第1个通道，同时裁剪/对齐到与每个分支卷积后的时间长度一致
        mask_in=mask[::,:1,::,::]
        mask=mask_in
        for i in range(len(self.kernel_set)):
            ...
            # 若时间维不一致，右侧截断较长的一方
            if feature_x.size(3) != mask_weight.size(3):
                min_t = min(feature_x.size(3), mask_weight.size(3))
                feature_x = feature_x[..., -min_t:]
                mask_weight = mask_weight[..., -min_t:]
            if self.temporal_partial_conv:
        ...
        x = torch.cat(x,dim=1)
        if self.temporal_mask_update:
            mask=torch.cat(mask_list,dim=1)
            mask,_=torch.max(mask,dim=1)
            mask=mask.unsqueeze(1)
        else:
            mask=mask_in[..., -x.size(3):]
        return x,mask
