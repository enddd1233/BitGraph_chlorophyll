...
def synthetic_data(mask_ratio,dataset):
    ...
    if(dataset=='Metr'):
        ...
        # 加载南海数据集
        df = pd.read_csv('./Mydata/nanhai_265.csv', index_col=0)
        df = df.drop(columns=['lat', 'lon'], errors='ignore')
        df = df[sorted(df.columns, key=lambda x: int(x))]
        data = df.to_numpy().T  # 转置：(时间步, 节点数)
        mask = get_0_1_array(data, mask_ratio)
...
def loaddataset(history_len,pred_len,mask_ratio,dataset):
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=32, shuffle=True, num_workers=0,
                                  drop_last=True)
    val_dataloader = DataLoader(val_dataset, batch_size=32, shuffle=False, num_workers=0, drop_last=True)
