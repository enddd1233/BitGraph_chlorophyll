...
class Model(nn.Module):
    def __init__(self, gcn_true, buildA_true, gcn_depth, num_nodes,kernel_set, device, predefined_A=None, static_feat=None, dropout=0.3, subgraph_size=5, node_dim=40, dilation_exponential=1, conv_channels=32, residual_channels=32, skip_channels=64, end_channels=128, seq_length=12, in_dim=2, out_len=12, out_dim=1, layers=3, propalpha=0.05, tanhalpha=3, layer_norm_affline=True, mask_topk=5, mask_bias_w=0.002, tri_bias_w=0.001, temporal_partial_conv=True, temporal_mask_update=True, spatial_mask_update=True):
        ...
        for i in range(1):
            ...
            for j in range(1,layers+1):
                ...
                self.filter_convs.append(dilated_inception(residual_channels, conv_channels,kernel_set, dilation_factor=new_dilation, temporal_partial_conv=temporal_partial_conv, temporal_mask_update=temporal_mask_update))
                self.gate_convs.append(dilated_inception(residual_channels, conv_channels,kernel_set, dilation_factor=new_dilation, temporal_partial_conv=temporal_partial_conv, temporal_mask_update=temporal_mask_update))
                ...
                if self.gcn_true:
                    self.gconv1.append(mixprop(conv_channels, residual_channels, gcn_depth, dropout, propalpha,dilationsize[j-1],num_nodes,self.seq_length,out_len, topk=mask_topk, mask_bias_w=mask_bias_w, tri_bias_w=tri_bias_w, spatial_mask_update=spatial_mask_update))
                    self.gconv2.append(mixprop(conv_channels, residual_channels, gcn_depth, dropout, propalpha,dilationsize[j-1],num_nodes,self.seq_length,out_len, topk=mask_topk, mask_bias_w=mask_bias_w, tri_bias_w=tri_bias_w, spatial_mask_update=spatial_mask_update))
    ...
    def forward(self, input,mask, k, idx=None):#tx,id
        ...
        if self.seq_length<self.receptive_field:
            pad_len = self.receptive_field - self.seq_length
            input = nn.functional.pad(input,(pad_len,0,0,0))
            # 同步对mask进行相同的时间维度左侧padding，确保后续卷积时间长度一致
            mask = nn.functional.pad(mask,(pad_len,0,0,0))
        if self.gcn_true:
